...
resource "azurerm_storage_account" "vm_sa" {
  count = var.boot_diagnostics && var.external_boot_diagnostics_storage == null ? 0 : 0  # was 1 : 0
...
resource "azurerm_backup_protected_vm" "vm_backup" {
  count = var.recovery_vault_name != "" ? var.nb_instances :  0
  
  resource_group_name = var.backup_vault_resource_group_name
  recovery_vault_name = var.recovery_vault_name
  source_vm_id        = local.is_windows ? azurerm_virtual_machine.vm_windows[count.index].id : azurerm_virtual_machine.vm_linux[count.index].id
